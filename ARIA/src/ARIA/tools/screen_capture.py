    ...
    try:
        ...
        if region:
            ...
            try:
                ...
                logger.warning("Geçersiz region formatı: %s", region)

        cmd.append(tmp_path)

        result = subprocess.run(cmd, capture_output=True, timeout=15)
        if result.returncode != 0:
            return {"success": False, "error": "screencapture başarısız"}
        ...
        return {"success": False, "error": "screencapture zaman aşımı"}
...
def analyze_screen(question: str = "Ekranda ne var?") -> dict:
    ...
    try:
        ...
        try:
            ...
            logger.error("Ollama vision hatası: %s", exc)
            return {
                "success": False,
                "error": f"Vision analizi başarısız: {exc}",
            }
